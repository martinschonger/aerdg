/*
Implementation of the update module that calls Charikar on the greedily constructed (size-constrained) neighborhood of the most recent request.
*/
...
namespace crep
{
	class comp_mig_greedy_age : public comp_mig_alg
	{
	public:
		comp_mig_greedy_age(
			const int_ num_hops,
		...
		std::vector<id_> get_mig_vtxs(graph_extended& g, const id_& src, const id_& dst, real_& sg_weight, const std::vector<int32_t>& parent = {}) override
		{
			...
			//Greedily construct the subgraph around the edge (src,dst)
			explore_greedy_deg(g, src, dst, num_hops, subg, g_to_subg, subg_to_g);
	...
	private:
		//Maximal number of vertices in the subgraph
		int_ num_hops;
