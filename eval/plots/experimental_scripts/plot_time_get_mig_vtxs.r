...
main_timestamp <- "20190822-234156"
specific_log_dir <- paste(log_directory, main_timestamp, "/", sep = "")

filenames <- list.files(specific_log_dir, pattern = "*time_get_mig_vtxs.csv")
...
max_x <- n
min_y <- 0
max_y <- 50#10000#good value for n=10000: 50

x_breaks <- seq(from = min_x, to = max_x, by = n/10)

y_breaks <- seq(from = min_y, to = max_y, by = max_y/10)

title <- paste("Cumulative time for computing mig set [", main_timestamp, "]", sep = "")
...
ggsave(paste(directory, "plot/", main_timestamp, "__time_get_mig_vtxs___", cur_timestamp, ".pdf", sep = ""), plot = p, dpi = "retina")
